...
double DonorCell::computeDu2Dx(int i, int j) const
{
    ...
    return (uMidRight * uMidRight - uMidLeft * uMidLeft + alpha_ * (fabs(uMidRight) * ((u(i, j) - u(i + 1, j)) / 2) - fabs(uMidLeft) * (u(i - 1, j) - u(i, j)) / 2)) / meshWidth()[0];
}
...
double DonorCell::computeDv2Dy(int i, int j) const
{
    ...
    return (vMidUp * vMidUp - vMidDown * vMidDown + alpha_ * (fabs(vMidUp) * ((v(i, j) - v(i, j + 1)) / 2) - fabs(vMidDown) * (v(i, j - 1) - v(i, j)) / 2)) / meshWidth()[1];
}
...
double DonorCell::computeDuvDx(int i, int j) const
{
    ...
    return (uMidUp * (v(i, j) + v(i + 1, j)) / 2 - uLeftUp * (v(i - 1, j) + v(i, j)) / 2 + alpha_ * (fabs(uMidUp) * (v(i, j) - v(i + 1, j)) / 2 - fabs(uLeftUp) * (v(i - 1, j) - v(i, j)) / 2)) / meshWidth()[0];
}
...
double DonorCell::computeDuvDy(int i, int j) const
{
    ...
    return (vMidRight * (u(i, j + 1) + u(i, j)) / 2 - vDownRight * (u(i, j) + u(i, j - 1)) / 2 + alpha_ * (fabs(vMidRight) * (u(i, j) - u(i, j + 1)) / 2 - fabs(vDownRight) * (u(i, j - 1) - u(i, j)) / 2)) / meshWidth()[1];
}