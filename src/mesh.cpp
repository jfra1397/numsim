...
void Mesh::set_boundary_condition(std::vector<Obstacle> objects)
{
    for(auto it = std::begin(objects); it != std::end(objects); ++it)
    {
        ...
        {
            for (int i = int(round(it->position[0]/physicalSize_[0]*nCells_[0])); i <= int(round(it->position[1]/physicalSize_[0]*nCells_[0])); i++)
            {
        ...
        {
            for (int i = int(round(it->position[0]/physicalSize_[1]*nCells_[1])); i <= int(round(it->position[1]/physicalSize_[1]*nCells_[1])); i++)
            {
        ...
        else if (it->type == EMPTY || it->type == FLUID)
        {
            ...
            {
                for (int j = int(ceil(it->position[0]/physicalSize_[1]*nCells_[1]))+1; j <= int(it->position[2]/physicalSize_[1]*nCells_[1]); j++)
                {
                    for (int i = int(ceil(it->position[1]/physicalSize_[0]*nCells_[0]))+1; i <= int(it->position[1]/physicalSize_[0]*nCells_[0]); i++)
                    {
                        flag_(i,j) = it->type;
