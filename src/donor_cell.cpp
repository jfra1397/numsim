...
double DonorCell::computeDu2Dx(int i, int j) const
{
    ...
    double result = (u_1*u_1 - u_2*u_2 + alpha_*(abs(u_1)*(u(i,j) - u(i+1,j) / 2) - abs(u_2)*(u(i-1,j) - u(i,j)) / 2)) / meshWidth_[0]
    return result;
}
...
double DonorCell::computeDuvDy(int i, int j) const
{
    ...
    double result = (v_1 * (u(i,j+1) + u(i,j)) / 2 - v_2* (u(i,j) + u(i,j-1)) / 2 + alpha_* (abs(v_1) * (u(i,j) - u(i,j+1)) / 2 - abs(v_2) * (u(i,j-1) - u(i,j)) /2))/meshWidth_[1];
    return result;
}