...
OutputWriterParaview::OutputWriterParaview(std::shared_ptr<Discretization> discretization, std::string outputPath, bool remove) : OutputWriter(discretization, outputPath, remove)
{
  // Create a vtkWriter_
  ...
  for (int j = 1; j < nCells[1] + 1; j++)
  {
    ...
    {
      if (discretization_->flag(i, j) != FLUID)
        dataSet->BlankCell(index);
    }
  }
}
...
void OutputWriterParaview::writeMesh()
{
  ...
  fileName << outputPath_ << "/generatedMesh"
