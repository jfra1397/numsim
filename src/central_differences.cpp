...
CentralDifferences::CentralDifferences(const std::array<int,2> nCells, const std::array<double,2> physicalSize):
...
{
    double result = ((v(i+1,j) + v(i,j)) * (u(i,j) + u(i,j+1)) - (v(i,j) + v(i-1,j)) * (u(i-1,j) + u(i-1,j+1)))/(4*meshWidth()[0]);
    return result;
}
...
{
    double result = ((u(i,j+1) + u(i,j)) * (v(i+1,j) + v(i,j)) - (u(i,j) + u(i,j-1)) * (v(i+1,j-1) + v(i,j-1)))/(4*meshWidth()[1]);
    return result;
}